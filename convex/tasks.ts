...
import { createHostedBamlDemoMissionArtifacts } from "../lib/bamlGtmPackage";
import type { BamlOpportunity, BamlSourceType } from "../lib/bamlGtmDemo";
...
export const createHostedBamlDemoMission = mutation({
    args: {
        opportunity: v.object({
            id: v.string(),
            sourceType: v.union(v.literal("X"), v.literal("Reddit"), v.literal("GitHub"), v.literal("HN"), v.literal("Paste")),
            sourceLabel: v.string(),
            sourceUrl: v.string(),
            discussion: v.string(),
            detectedPain: v.string(),
            bamlRelevance: v.string(),
            confidence: v.number(),
            channelFit: v.string(),
    ...
    handler: async (ctx, args) => {
        const opportunity: BamlOpportunity = {
            ...args.opportunity,
            sourceType: args.opportunity.sourceType as BamlSourceType,
        };
        const now = Date.now();
        const artifacts = createHostedBamlDemoMissionArtifacts(opportunity, now);
        const taskId = await ctx.db.insert("tasks", {
            title: artifacts.title,
            description: artifacts.description,
            priority: "high",
            status: "done",
            workflow: ["Porter", "Torvalds", "Ogilvy", "Carnegie"],
        ...
        await ctx.db.insert("activity", {
            ...
            type: "success",
            content: `Hosted demo package generated for ${opportunity.sourceLabel}.`,
            timestamp: now,
        });
        await ctx.db.insert("activity", {
            agentName: "Tigerclaw",
            type: "success",
            content: `BAML Opportunity Package ready for human review: ${opportunity.detectedPain}`,
            timestamp: now + 1,
        });
...
export const recordFinalApproval = mutation({
    args: {
        id: v.id("tasks"),
        status: v.string(),
    ...
    handler: async (ctx, args) => {
        await ctx.db.patch(args.id, {
            finalApprovalStatus: args.status,
            finalApprovalNote: args.note,
            finalApprovalAt: Date.now(),
        });

        await ctx.db.insert("activity", {
            agentName: "Human",
            type: "action",
            content: `Final response marked: ${args.status}`,
            timestamp: Date.now(),
        });
    },
});

export const recordOutcomeFeedback = mutation({
    args: {
        id: v.id("tasks"),
        relevance: v.number(),
        timing: v.number(),
    ...
    handler: async (ctx, args) => {
        const outcomeFeedback = {
            ...
            timing: args.timing,
            technicalUsefulness: args.technicalUsefulness,
            credibility: args.credibility,
            engagement: args.engagement,
            opportunityQuality: args.opportunityQuality,
            risk: args.risk,
            notes: args.notes,
            recordedAt: Date.now(),
        };
        ...
        await ctx.db.insert("activity", {
            agentName: "Tigerclaw",
            type: "log",
            content: `Post-outcome feedback recorded for BAML radar mission.`,
            timestamp: Date.now(),
        });
    },
});
...
export const handoff = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (nextStep >= task.workflow.length) {
            // Special case: Porter-only strategy tasks may need raw output.
            // Skip Tigerclaw review/synthesis to prevent tab wrapping.
...
`;
            ...
            const isBamlDemoTask = Boolean(task?.description?.includes("BAML_GTM_RADAR_DEMO"));
            if (!isBamlDemoTask) {
                await ctx.scheduler.runAfter(0, api.memoryNode.storeMemory, {
