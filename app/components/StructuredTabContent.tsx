    ...
    return (
        <ReactMarkdown
            ...
            components={{
                ...
                table: ({ children }) => (
                    <div className="my-6 overflow-x-auto rounded-2xl border border-white/10 bg-black/20">
                        <table className="w-full min-w-[640px] border-collapse text-left text-sm">{children}</table>
                    </div>
                ),
                thead: ({ children }) => (
                    <thead className="bg-white/[0.06] text-xs uppercase tracking-widest text-gray-400">{children}</thead>
                ),
                th: ({ children }) => (
                    <th className="border-b border-white/10 px-4 py-3 font-black">{children}</th>
                ),
                td: ({ children }) => (
                    <td className="border-b border-white/5 px-4 py-3 align-top text-gray-200 last:border-b-0">{children}</td>
                ),
                p: ({ children }) => {
...
export default function StructuredTabContent({ content, agentName, activeTabId }: { content: string, agentName: string, activeTabId?: string }) {
    const markdownOnlyTabs = new Set([
        "posting_payload",
        "opportunity",
        "technical_proof",
        "agent_harness",
    ...
    if (activeTabId && markdownOnlyTabs.has(activeTabId)) {
        return (
            <div className={READING_PROSE}>
                <EnhancedMarkdown content={content} />
            </div>
    ...
    if (parsed) {
        ...
        if (parsed.type === 'scout') return <ScoutView data={parsed.data as ScoutOutput | ScoutCandidate[]} />;
