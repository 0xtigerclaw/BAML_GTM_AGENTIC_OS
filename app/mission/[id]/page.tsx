...
import PostOutcomeFeedback from "../../components/PostOutcomeFeedback";
...
function isBamlRadarTask(title?: string, description?: string): boolean {
    return Boolean(
        description?.includes("BAML_GTM_RADAR_DEMO") ||
        title?.includes("BAML Opportunity Radar") ||
        title?.includes("BAML Developer Opportunity"),
    );
}
...
export default function MissionDetailPage() {
    ...
    const agentNameForStructuredTab = useMemo(() => {
        ...
        if (tab.id === "evaluation") return "Tigerclaw";
        if (tab.id === "posting_payload") return "Tigerclaw";
        if (tab.id === "opportunity") return "Porter";
        if (tab.id === "technical_proof") return "Torvalds";
        if (tab.id === "response_drafts") return "Ogilvy";
        if (tab.id === "qa") return "Carnegie";
        if (tab.id === "agent_harness") return "System";
    ...
    const isBamlTask = isBamlRadarTask(task.title, task.description);
    const bamlExtensions = task as unknown as BamlTaskExtensions;
    ...
    if (nextSteps.length === 0 && task.status === "done") {
        nextSteps = isBamlTask
            ? [
                "Tighten the final response",
                "Make the technical proof shorter",
                "Adapt this for Reddit"
            ]
            : [
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-black pb-24">
            ...
            <main className={cn(
                ...
                {showFinalPanel && (
                    <div ref={finalOutputRef} className="rounded-2xl border border-gray-800 bg-[#0f1117] shadow-2xl overflow-hidden ring-1 ring-white/5">
                            ...
                            if (tabsData) {
                                return (
                                    <div className="flex items-center gap-4 p-6 bg-gradient-to-r from-emerald-500/20 to-teal-900/20 text-emerald-100 border-b border-white/5">
                                        ...
                                        <div>
                                            <h2 className="text-xl font-bold tracking-tight">{isBamlTask ? "BAML Opportunity Package" : "Mission Report"}</h2>
                                            <p className="text-sm opacity-60 font-medium">
                                                {task.status === "done" ? "Final synthesized output" : "Live output"} • human-reviewed GTM response workflow
                                            </p>
                                        </div>
                                    </div>
                                );
                                            ...
                                            && overlayHookCandidates.length > 0 && (
                                                <div className="p-6 rounded-2xl border border-amber-500/20 bg-amber-500/5">
                                                    ...
                                                    <div className="mt-4 text-xs text-gray-400">
                                                        After you pick one, Mission Control hands off to Ive and the Designer output appears automatically.
                                                    </div>
                                                </div>
                                            )}
                        ...
                        <div className="px-8 py-4 bg-gray-950/50 border-t border-gray-800 flex justify-between items-center text-xs">
                            <p className="text-gray-600 font-medium tracking-wide uppercase">Mission Control • Secure Transmission</p>
                ...
                {isBamlTask && task.status === "done" && (
                    <PostOutcomeFeedback
                        taskId={taskId}
                        finalApprovalStatus={bamlExtensions.finalApprovalStatus}
                        finalApprovalNote={bamlExtensions.finalApprovalNote}
                        outcomeFeedback={bamlExtensions.outcomeFeedback}
                    />
                )}
