...
Usage:
  ...
  ./start.sh toggle          Start if stopped, stop if running
...
case "$cmd" in
  "" )
    ensure_deps
    echo "🚀 Launching Mission Control Stack (foreground)..."
  ...
  "--detach"|"-d"|"detach"|"start" )
    ...
    if is_running; then
      ...
      echo "🚀 Launching Mission Control Stack (background)..."
